			...
			class CreateVm < ProxmoxAction
				...
				def create_params_lxc(config, env, vm_id)
					{vmid: vm_id,
					 ...
					 description: "#{config.vm_name_prefix}#{env[:machine].name}"}
					.tap do |params|
						params[:net0] = "name=#{get_machine_interface_name(env)},ip=#{get_machine_ip_address(env)}/28,gw=#{get_machine_gw_ip(env)},bridge=#{get_machine_bridge_name(env)}" if get_machine_ip_address(env)
					end
				end
			end
		end
